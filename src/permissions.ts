...
export const buildPermissions = async (compiler: Compiler): Promise<Permissions[]> => {
  ...
  return await new Promise<Permissions[]>((resolve) => {
    compiler.hooks.normalModuleFactory.tap('ChromeManifestGenerator', factory => {
      const handler = (parser: javascript.JavascriptParser): void => {
        ...
        parser.hooks.callMemberChain.for('chrome').tap('ChromeManifestGenerator', (_, chain) => {
          if (chain.length >= 1 && chain[0] === 'storage') {
            permissions.add('storage')
          }
        })

        parser.hooks.callMemberChain.for('navigator').tap('ChromeManifestGenerator', (_, chain) => {
          if (chain.length >= 1 && chain[0] === 'geolocation') {
